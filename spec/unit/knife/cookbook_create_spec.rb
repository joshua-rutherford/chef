...
describe Chef::Knife::CookbookCreate::Context do

  let(:context) { described_class.new(hash) }
  let(:expected) { values.map { |value| value }.join("\n") }
  let(:erb) { ERB.new(template) }
  let(:hash) { size.times.reduce({}) { |hash, index| hash[keys[index].to_sym] = values[index]; hash  } }
  let(:keys) { size.times.map { "a#{SecureRandom.hex}" } }
  let(:size) { 10 }
  let(:template) { keys.map { |key| "<%= #{key} %>" }.join("\n") }
  let(:values) { size.times.map { SecureRandom.hex } }

  describe '.new' do

    subject { context }

    it { should be_a Chef::Knife::CookbookCreate::Context }
  end

  describe '#evaluate' do

    subject { context.evaluate(erb) }
